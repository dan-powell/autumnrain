...
<section class="Show-inner">
    <div class="Show-aside">
        <div class="Show-aside-inner">
            @if($previous)
                @include('projects.aiwork.components._pager', ['piece' => $previous, 'class' => '_prev'])
            @endif
            {{-- <a class="Show-logo" href="/">
                <img src="{{ asset('assets/aiwork_logo.svg') }}" class="Show-logo-img"/>
            </a> --}}
            <div class="Show-aside-mid">
                <div class="Show-themes">
                    @foreach($piece->themes as $theme)
                        <h2 class="Show-theme-week">Week {{ $theme->created_at->format('W') }}</h2>
                        <h2 class="Show-theme-name">{{ $theme->name }}</h2>
                    @endforeach
                </div>
            ...
            @if($first)
                @include('projects.aiwork.components._pager', ['piece' => $first, 'class' => '_first'])
            @endif
        </div>
    </div>
        ...
        @if(isset($piece->metadata_json) && count($piece->metadata_json) > 1)
            <details class="Show-metadata">
                ...
                <dl class="Show-metadata-list">
                    @each('projects.aiwork.components._metadata', $piece->metadata_json, 'metadata')
                </dl>
            </details>
        @endif
    </section>
    <div class="Show-aside _last">
        <div class="Show-aside-inner">
            ...
            @endif
            @if($next)
                @include('projects.aiwork.components._pager', ['piece' => $next, 'class' => '_next'])
            @endif
            @if($last)
                @include('projects.aiwork.components._pager', ['piece' => $last, 'class' => '_last'])
            @endif
        </div>
    </div>
</section>
